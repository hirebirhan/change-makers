  ...
  if (loading) {
    return (
      <div className="flex-1 flex items-center justify-center min-h-[60vh]">
        <p className="text-sm text-muted-foreground animate-pulse">Loading videos…</p>
      </div>
    );
  }
      ...
      <main className="max-w-7xl mx-auto px-6 py-8 space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center gap-4">
          <div>
            ...
            <p className="text-sm text-muted-foreground">{data?.videos.length ?? 0} videos</p>
          </div>
          ...
          {filtered.map((video) => (
            ...
            >
              <div className="relative aspect-video bg-muted">
                {video.thumbnailUrl ? (
                  <Image src={video.thumbnailUrl} alt={video.title} fill className="object-cover" sizes="320px" />
                ) : (
